...
export default function SearchScreen() {
  ...
  const [selectedCategory, setSelectedCategory] = useState("학과공지");
  ...
  const [apiUrlCache, setApiUrlCache] = useState<Record<string, string>>({});
  ...
  const fetchApiUrl = async (category: string): Promise<string> => {
    ...
    try {
      if (category === "학과공지") {
        ...
        if (deptSnap.exists()) url = deptSnap.data().apiUrl;
      } else {
        ...
        const docId = keyMap[category];
        const noticeRef = doc(db, "notice", docId);
        const noticeSnap = await getDoc(noticeRef);
        if (noticeSnap.exists()) url = noticeSnap.data().apiUrl;
      }
  ...
  const handleSearch = async (customQuery?: string) => {
    ...
    try {
      ...
      const filtered = data.filter((item: NoticeItem) =>
